...
local function GetLootBoxes()
    local Boxes = {}

    for _, Child in workspace.Map:GetChildren() do
        if Child.Name == 'LootCrate' or Child.Name == 'LootBox' then
            table.insert(Boxes, Child)
        end
    end

    for _, Child in workspace.SECTOR_3.Sector3Props:GetChildren() do
        if Child.Name == 'LootCrate' or Child.Name == 'LootBox' then
            table.insert(Boxes, Child)
        end
    end
...
local Characters = GetLootBoxes()
local CurrentIndex = 1

game:GetService('RunService').RenderStepped:Connect(function()
    Characters = GetLootBoxes()
...
leftButton.MouseButton1Click:Connect(function()
	...
	if Characters[NextIndex] then
        local Case = Characters[NextIndex]:FindFirstChild('Case')
        local Root = Case and Case:FindFirstChild('CrateRoot')

        CurrentSubject = Root or game.Players.LocalPlayer.Character.Humanoid
    end
end)
